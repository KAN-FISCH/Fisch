...
local function Init(AutosCollect, AutosQuest, AutosJack, AutosFavorit, AutosAppraise, AutoAppraise, AutoEnchant, Collect, AutosSection, AuraSection, AutoJetskiSection, FoodSection)
    ...
    appraiseOnHeldToggle = AutoAppraise:AddToggle({
        ...
        Callback = function(value)
            ...
            task.spawn(function()
                ...
                task.wait(0.2)
                pcall(function() DialogInteract:InvokeServer(choiceNode, 1) end)
                task.wait(0.5)
                ...
                while Appraise do
                    ...
                    local oldData = getEquippedItemData(targetItemId)
    ...
    ---------------------------------------------------------
    -- 2. AutoEnchant Section
    ---------------------------------------------------------
    local whileEnchant = false
    local targetEnc = {}
    local enchanList = {
        "Frightful", "Blessed Song", "Sea King", "Swift", "Long", "Ghastly", "Lucky",
        "Divine", "Mutated", "Unbreakable", "Steady", "Blessed", "Wormhole", "Resilient",
        "Controlled", "Storming", "Scrapper", "Breezed", "Insight", "Clever", "Noir",
        "Hasty", "Quality", "Abyssal", "Herculean", "Mystical", "Quantum", "Invincible",
        "Piercing", "Anomalous", "Sea Overlord", "Immortal", "Flashline", "Scavenger",
        "Chaotic", "Blood Reckoning", "Chronos", "Momentum", "Overclocked", "Tenacity",
        "Tryhard", "Wise", "Cryogenic", "Sea Prince", "Glittered"
    }

    local function hasSameEnchant()
        local RodName = workspace.PlayerStats[LocalPlayer.Name].T[LocalPlayer.Name].Stats.rod.Value
        local enchantContainer = LocalPlayer.PlayerGui.hud.safezone.equipment.rods.scroll.safezone[RodName].rod.enchants
    ...
    AutoEnchant:AddToggle({
        ...
        Callback = function(value)
            whileEnchant = value
            task.spawn(function()
                local enchant = ReplicatedStorage:WaitForChild("events"):WaitForChild("enchant")
                ...
                if value then
                    ...
                    task.spawn(function()
                        while whileEnchant do
                            local cycleObj = ReplicatedStorage:FindFirstChild("world") and ReplicatedStorage.world:FindFirstChild("cycle")
                ...
                while AutoAngler do
                    ...
                    local function getQuest()
                        ...
                        if DataController then
                            local data = nil
                            -- Coba PlayerDataReplicator secara aman tanpa yield
                    ...
                    local function triggerAnglerPrompt()
                        ...
                        -- 1. Cari via CollectionService (sesuai tag game asli "NewNpc")
                    ...
                    if targetFish then
                        ...
                        if fishTool then
                            ...
                            -- Disable AutoCast, AutoReel, and AutoEquipRod temporarily for safe quest turn-in
                            ...
                            -- Unequip rod/tools before teleporting
                    ...
                    if not fishActioned and detected then
                        ...
                        -- Fallback: Jika tidak terdeteksi spot spesifik, gunakan spot pancing di area Angler saat ini
                        ...
                        if spotDetected then
                            if not _G.Config.selectedZoneADS then
                                ...
                                -- Auto-activate fishing features upon teleport
                                task.spawn(function()
