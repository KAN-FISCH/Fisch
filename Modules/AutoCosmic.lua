    ...
    if success then
        ...
        if ok and data and data.rows and type(data.rows) == "table" then
            for _, row in ipairs(data.rows) do
                if row.payload and row.payload.embeds then
...
local function hasCosmic(parent, depth)
    ...
    for _, child in ipairs(parent:GetChildren()) do
        if child.Name == "Cosmic Relic" or child.Name == "Starfall Totem" or child.Name == "Lunar Thread" or child.Name == "Meteoric" then
            local prompt = nil
    ...
    while _G.Config and _G.Config.AutoHopCosmic do
        ...
        if foundCosmic then
            ...
            for _, srv in ipairs(servers) do
                ...
                if jobId and placeId and not blacklistedServers[jobId] then
            ...
            if targetServer then
                ...
                createHopNotification("No Cosmic", "API has no cosmic servers", Color3.fromRGB(155, 89, 182))
                task.wait(5)
            end
        end
    end
...
local function SetEnabled(state)
    if state then
        ...
        _G.Config.AutoClaimMulti = true
        if getgenv().startAutoClaimMulti then
            task.spawn(getgenv().startAutoClaimMulti)
        end
