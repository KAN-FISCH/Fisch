...
local function resetState(bypassCooldown)
    ...
    if bobberRef and bobberRef.Parent then
        pcall(function()
            local existing = bobberRef:FindFirstChild("ShieldBobberLock")
            if existing then existing:Destroy() end
            local existingAtt = bobberRef:FindFirstChild("ShieldBobberAtt")
            if existingAtt then existingAtt:Destroy() end
            -- Restore normal collision so the bobber can land on water
            bobberRef.CanCollide = true
        end)
    end
    bobberRef = nil
...
local function startLoop()
    ...
    heartbeatConn = RunService.Heartbeat:Connect(function()
        ...
        if char:GetAttribute("Reeling") then
            -- Cek apakah reel UI atau shake UI aktif. Jika tidak, dan tidak ada bobber, maka Reeling stuck!
            ...
            if not isActivelyReeling then
                -- Stuck! Force clear attribute
                pcall(function()
        ...
        local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
        local reelGui = playerGui and playerGui:FindFirstChild("reel")
        if reelGui and reelGui.Enabled then return end
        
        if tick() - (_G.LastCatchTick or 0) < 0.01 then return end
        ...
        if state == S_CAST then
            if not castPending then
                doCast(rod, hrp)
            elseif now - lastCastTick > 0.4 then
                -- Watchdog: casting stuck
                castPending = false
            end

        elseif state == S_WAIT then
            -- High Lure Speed Protection: Jika ikan menggigit saat kail masih di S_WAIT, langsung transisi ke S_LOCK
            local shakeui = playerGui and playerGui:FindFirstChild("shakeui")
            if char:GetAttribute("Reeling") or (shakeui and shakeui.Enabled) then
                state = S_LOCK
                lockStartTick = tick()
                return
            end
            ...
            if not bobberHandled then
                ...
                elseif now - lastCastTick > 1.2 then
                    -- timeout watchdog: hanya reset jika player TIDAK sedang reeling
