...
-- ─────────────────────────────────────────────────────────
-- Floating Protected GUI (Anti-Detection System like Kaitun)
-- ─────────────────────────────────────────────────────────
...
local function SafeProtectGUI(gui, customName)
    if not gui then return end
    gui.Name = customName or ("RobloxGui_" .. string.sub(HttpService:GenerateGUID(false), 1, 8))
...
function Library:SetNotification(notifConfig)
    ...
    if not NotificationGui or not NotificationGui.Parent then
        ...
        SafeProtectGUI(NotificationGui, "RobloxGui_Notif")
    end
...
function Library:CreateWindow(config)
    ...
    if getgenv().__NewFish5_ScreenGui then
        pcall(function() getgenv().__NewFish5_ScreenGui:Destroy() end)
    end

    local ScreenGui = Instance.new("ScreenGui")
    SafeProtectGUI(ScreenGui)
    getgenv().__NewFish5_ScreenGui = ScreenGui
